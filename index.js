...
const mongoURL =
    "mongodb+srv://florixer:Kau93043@flexomate-cluster.bzqxpj3.mongodb.net/users";
  ...
  try {
    ...
    res.status(500).json({ message: "Failed to add continents collections" });
        ...
        app.get("/users", async (req, res) => {
            ...
            try {
                // Search for users in all collections
                ...
                const users = await Promise.all(
                    collections.map(async (collection) => {
                        try {
                            ...
                            console.error(
                                `Error finding users in collection ${collection.name}:`,
                                error
                            );
                            return [];
                        }
        ...
        app.get("/spotify/download/:id", async (req, res) => {
            try {
                ...
                console.error(
                    "Error making external API request:",
                    error.message
                );
                res.status(500).json({
                ...
                const { data } = await axios.get(
                    "https://spotify-downloader1.p.rapidapi.com/trackList/playlist/37i9dQZEVXbLZ52XmnySJg",
                    {
                        headers: {
                            ...
                            "X-RapidAPI-Host":
                                "spotify-downloader1.p.rapidapi.com",
                        },
                    }
                );
